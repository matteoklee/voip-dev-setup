...
def ksr_request_route():
    KSR.info("===== request route - from kamailio kemi/python script\n")
...
def ksr_route_relay():
    ...
    #RTPENGINE
    #if KSR.is_content_type("application/sdp"):
    
    #RTPENGINE
    #if KSR.pv.get("$ct") == "application/sdp":
        #if KSR.msgops.search_body("a=rtpmap:96 opus/48000/2"):
            #KSR.rtpengine.rtpengine_manage("codec-transcode-PCMU")
        #else:
            #KSR.rtpengine.rtpengine_manage("codec-transcode-opus")
...
def ksr_route_withindlg():
    ...
    #if KSR.is_method("ACK|BYE"):
        #KSR.pv.set("$duri", KSR.pv.get("$ruri"))
        #if KSR.tm.t_relay() < 0:
            #KSR.sl.sl_send_reply(500, "Server error")
        #return 0
...
def ksr_onreply_manage():
    ...
    #RTPENGINE
    #if KSR.pv.get("$ct") == "application/sdp":
        #KSR.rtpengine.rtpengine_manage("codec-mask-all codec-transcode-opus")
